...
export async function listDocumentsForOrg(orgId: string): Promise<ProjectDocument[]> {
  const { data, error } = await createAdminClient()
    .from("documents")
    .select("*")
    .eq("org_id", orgId)
...
export async function deleteDocument(orgId: string, id: string): Promise<void> {
  const { error } = await createAdminClient().from("documents").delete().eq("id", id).eq("org_id", orgId);
  if (error) throw error;
}
